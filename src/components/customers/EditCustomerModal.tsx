...
export function EditCustomerModal({
    ...
    try {
      ...
      if (!res.ok) {
        const msg =
          ...
          `Failed to update customer (${res.status})`;
        setError(msg);
        setIsLoading(false);
        return;
      }
  ...
  return (
    <Modal
      ...
      onClose={onClose}
      title="Edit Customer"
      subtitle="Update customer record"
      ...
      <form onSubmit={handleSubmit} className="space-y-5" noValidate>
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          <Field label="Lifecycle stage">
            <select
              value={values.lifecycleStage ?? "prospect"}
              onChange={(e) =>
                update("lifecycleStage", e.target.value as LifecycleStage)
              }
              disabled={isLoading}
              className={inputClass}
            >
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-4">
            <ConsentCheckbox
              id="edit-consentSms"
              label="SMS"
              checked={values.consentSms ?? true}
              disabled={isLoading}
              onChange={(v) => update("consentSms", v)}
            />
            <ConsentCheckbox
              id="edit-consentEmail"
              label="Email"
              checked={values.consentEmail ?? true}
              disabled={isLoading}
              onChange={(v) => update("consentEmail", v)}
            />
            <ConsentCheckbox
              id="edit-consentPhone"
              label="Phone"
              checked={values.consentPhone ?? true}
              disabled={isLoading}
