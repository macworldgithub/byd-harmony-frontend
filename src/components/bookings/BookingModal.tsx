...
export function BookingModal({
  ...
  const [serviceType, setServiceType] = useState<BookingServiceType>("routine");
  ...
  useEffect(() => {
    ...
    if (isEdit && booking) {
      ...
      setServiceType(booking.serviceType || "routine");
      setStatus(booking.status || "scheduled");
      setNotes(booking.description || booking.notes || "");
      setEstimatedDuration(booking.estimatedDuration || "");
      const dateVal = booking.scheduledAt || booking.serviceDateTime;
      if (dateVal) {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      if (isEdit) {
        ...
        payload = {
          serviceType,
          scheduledAt: new Date(serviceDateTime).toISOString(),
